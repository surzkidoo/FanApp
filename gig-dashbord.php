<?php session_start();
$connect = new mysqli("localhost:3306","secretgist_Orbvibeser2","Pass@123@1", "secretgist_Orbvibes2");
...
$offset=$limit*$page;
    ...
    $sql="SELECT * from gig where user_id=$user order by gig_id desc limit $offset,$limit";
...
bootstrap.min.css">
<link rel="stylesheet" href="article-dashboard.css">
...
<nav class="navbar navbar-default navbar-expand-lg  navbar-light">
    ...
    <div class="white fcontainer">
        ...
        <div class="container">
            <p id="title" class="label mb-4"><?php echo $username;?></p>
            <div class="menu">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-8 mt-sm-2">
                ...
                <a href="article.php" class="btn btn-success mt-3 mb-3">Create Article</a><br>
                <div class="col-12 col-sm-12 col-md-12 mx-auto">
                    <div class="table-responsive">
                       
                        <table class="table table-striped table-hover">
                            <th>Gig Image</th><th>Gig title</th><th>Edit<th>Delete</th>
                            <?php   
                           while($row=$resulta->fetch_assoc()){
                        ?>
                            <tr>
                                <td><img src="<?php echo $row["post_img"]?>" class="" width="100px" alt="">
                                </td>
                                <td><p><?php echo $row["post_title"]?></p></td>
                              
                                <td><a href="article-edit.php?id=<?php echo $row["post_id"]?>" class="btn btn-primary">Edit</a></td>
                                <td><a href="article-delete.php?id=<?php echo $row["post_id"]?>" class="btn btn-danger">Delete</a></td>
                            </tr>
                           <?php }?>
                            </table>
...
$rows= $result_num->num_rows;
$rows =ceil($rows/=$limit);
for($i=1;$i<=$rows;$i++){
    $newi=$i-1;
    if($newi==$page)
    
    echo"<a id='active' href='gig-dashboard.php?page=$newi'>$i</a>";
    else
    echo"<a id='' href='gig-dashboard.php?page=$newi'>$i</a>";
}
