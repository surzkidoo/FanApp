...
<?php 
	...
	$sql="SELECT * from post left join user on post.user_id = user.id  where post.post_id=$id";
	$result=$connect->query($sql);
...
<div class="container">
<h4><?php echo$gig["event_name"]?></h4>
    <div class="row">
            
        <div class="col-12 col-sm-8 mx-auto col-md-6">
            <img src="<?php echo $gig["event_pic"] ?>" width="100%" alt="event picture">
        </div>
        <div class="col-12 col-sm-8 mxauto col-md-6">
                <?php 
                $country=$gig['country'];
               $sql="select * from country where country_id=$country";
               $result=$connect->query($sql);
               $gigc=$result->fetch_assoc();
                ?>
       <img src="<?php echo $gigc["image_link"] ;?>" width="25px" alt="country image">
        <span class=""><?php echo $gigc["name"] ; ?></span>
        <p><?php echo $gig["date"]," ".$gig["time"];?></p>
        <p><?php echo $gig["venue"] ;?></p>
        <p><?php echo$gig["event_desc"]?></p>
       <?php if(!empty($gig["website"])){ ?>
        <a href="<?php echo $gig["website"];?>">Visit Website</a>
       <?php }?>
        </div>
